...
export const headerFormat = (format) => {
    return HEADER_FORMAT[format];
};
export const dayCalculations = (format) => {
    return DAY_CALCULATIONS[format];
};
export const ganttUnitWidth = (format) => {
    return UNIT_WIDTH[format];
};
...
export const getDirectionMultiplier = (goingDown) => goingDown ? -1 : 1;
export const getVerticalOffset = (baseY, direction, offset) => {
